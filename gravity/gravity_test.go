...
func TestTotalGravitationalForce(t *testing.T) {
	...
	want := cp.Vector{X: 0.10025}
	if got := g.TotalGravitationalForce(space, shapes[0].Body()); !reflect.DeepEqual(got, want) {
		t.Errorf("TotalGravitationalForce shape 0: got %v want %v", got, want)
	}
	want = cp.Vector{}
	if got := g.TotalGravitationalForce(space, shapes[1].Body()); !reflect.DeepEqual(got, want) {
		t.Errorf("TotalGravitationalForce shape 1: got %v want %v", got, want)
	}
	want = cp.Vector{X: -0.10025}
	if got := g.TotalGravitationalForce(space, shapes[2].Body()); !reflect.DeepEqual(got, want) {
		t.Errorf("TotalGravitationalForce shape 2: got %v want %v", got, want)
	}
}
...
func Test_gravitationalForce(t *testing.T) {
	...
	f := gravitationalForce(earth, sun, G)
